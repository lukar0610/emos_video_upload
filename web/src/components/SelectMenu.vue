<script lang="ts" setup>
  ...
  function updateMenuPosition() {
    ...
    const edgePadding = 8
    const left = Math.min(Math.max(edgePadding, triggerRect.left), Math.max(edgePadding, window.innerWidth - width - edgePadding))
    const menuHeight = Math.min(optionsElement.value?.scrollHeight ?? 280, 280)
    const openAbove = window.innerHeight - triggerRect.bottom < menuHeight + edgePadding && triggerRect.top > menuHeight + edgePadding
...
<template>
  <div ref="root" :class="{ 'opacity-60': disabled }" class="relative min-w-0">
    <button
      ...
      :disabled="disabled"
      :class="[{ 'border-[#8bbec8] shadow-[0_0_0_3px_rgb(32_111_131/8%)]': open }, props.compact ? 'min-h-8 gap-2 rounded-[4px] px-2 py-1' : 'min-h-[42px] gap-3 rounded-md px-[11px] py-[7px]']"
      class="flex w-full items-center justify-between border border-[#ccdce1] bg-white text-left text-ink transition hover:border-[#8bbec8] hover:shadow-[0_0_0_3px_rgb(32_111_131/8%)] disabled:cursor-not-allowed"
      type="button"
      @click.stop="toggle">
      <span class="grid min-w-0 gap-0.5">
        <strong :class="props.compact ? 'text-[11px]' : 'text-xs'" class="truncate font-bold text-[#30434b]">{{ selectedOption()?.label || placeholder }}</strong>
      ...
      <span
        ...
        aria-hidden="true"
        class="size-2 shrink-0 rotate-45 border-r-[1.5px] border-b-[1.5px] border-[#6d8087] transition-transform"></span>
    </button>
    <Teleport to="body">
      <div
        v-if="open"
        ...
        :aria-label="ariaLabel"
        :style="menuStyle"
        class="fixed z-[100] grid max-h-[280px] min-w-[190px] gap-[3px] overflow-auto rounded-[7px] border border-line bg-white p-[5px] shadow-[0_12px_28px_rgb(30_54_64/16%)]"
        role="listbox">
        <input
          v-if="options.length > 8"
          v-model="query"
          class="mx-1.5 my-1.5 w-[calc(100%_-_12px)] rounded border border-line px-2 py-[7px] text-xs outline-none focus:border-accent"
          placeholder="快速搜索"
          @click.stop />
        <button
