<script lang="ts" setup>
  ...
  const { authReady, loginBusy, loginError, loginUsername, loginPassword, isAuthenticated, authEnabled, checkAuth, login, logout } = useAuth()
...
<template>
  <main class="min-h-screen bg-canvas">
    ...
    <section v-else-if="authEnabled && !isAuthenticated" class="grid min-h-screen place-items-center p-6">
      <form class="grid w-full max-w-[360px] gap-4 rounded-lg border border-line bg-white p-9 shadow-[0_16px_40px_rgb(35_54_64/8%)]" @submit.prevent="login">
        ...
        <label class="grid gap-2 text-[13px] font-semibold text-[#5f717b]">
          <span>用户名</span>
          <input v-model="loginUsername" autocomplete="username" autofocus class="min-h-[42px] w-full rounded-md border border-[#ccdce1] bg-[#fbfdfe] px-3 text-ink outline-none focus:border-accent" />
        </label>
        <label class="grid gap-2 text-[13px] font-semibold text-[#5f717b]">
          <span>密码</span>
          <input
        ...
        <button
          :disabled="loginBusy"
          class="min-h-10 w-full rounded-md bg-accent px-4 text-[13px] font-bold text-white shadow-[0_2px_5px_rgb(32_111_131/16%)] transition hover:bg-accent-dark hover:shadow-[0_4px_9px_rgb(32_111_131/20%)]">
          {{ loginBusy ? '登录中…' : '登录' }}
        </button>
      </form>
    </section>
    ...
    <template v-else>
      ...
      <div
        class="mx-auto mt-[18px] mb-16 grid w-[min(1480px,calc(100%_-_36px))] items-start gap-[18px] pb-4 lg:mb-0 lg:h-[calc(100vh-86px)] lg:min-h-0 lg:grid-rows-[minmax(0,1fr)] lg:grid-cols-[minmax(300px,0.78fr)_minmax(0,1.6fr)] lg:items-stretch lg:overflow-hidden max-lg:mb-20 max-lg:w-[min(calc(100%_-_20px),1480px)] max-lg:gap-3">
        <SourceBrowser @select-file="handleSelectFile" @select-directory="handleSelectDirectory" />
        <section class="flex min-w-0 flex-col gap-[18px] lg:h-full lg:min-h-0 lg:overflow-y-auto lg:pr-1">
