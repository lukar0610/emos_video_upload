...
export function useVideoInspector(file: Ref<SourceFile | null>) {
    ...
    try {
      ...
      if (autoMatchedEpisode) {
        await chooseTarget({
          ...
          has_media: autoMatchedEpisode.episode.has_media,
  ...
  function chooseEpisode(parent: VideoTreeItem, seasonNumber: number, episodeNumber: number, itemId: number, episodeTitle: string, hasMedia: boolean) {
    void chooseTarget({
      item_type: 've',
      item_id: itemId,
