...
const currentPathStorageKey = 'emos_video_upload.current_path'

export function useSourceBrowser() {
  const storedCurrentPath = useSessionStorage(currentPathStorageKey, '')
  ...
  async function loadDirectory(path: string) {
    ...
    try {
      ...
      storedCurrentPath.value = data.path
  ...
  async function initialize() {
    const storedPath = storedCurrentPath.value
    if (!(await loadDirectory(''))) {
      return
    }
    if (storedPath && storedPath !== currentPath.value) {
      if (await loadDirectory(storedPath)) {
        await scanCurrentDirectory()
        return
      }
      await loadDirectory('')
