...
export function useBatchInspector(directory: Ref<DirectorySelection | null>) {
  ...
  const missingBaseInfoAssignments = computed(() =>
    matchedAssignments.value.filter((assignment) => assignment.target?.has_media === true && assignment.baseInfo === null && !assignment.baseLoading),
  )
  ...
  async function loadBaseInfo(assignment: BatchAssignment) {
    if (!assignment.target || assignment.target.has_media !== true) {
      assignment.baseInfo = null
