...
func validTVTaskNumbers(seasonNumber, episodeNumber int) bool {
...
func (s *Server) handleCreateBatchTasks(w http.ResponseWriter, r *http.Request) {
	...
	for _, item := range input.Tasks {
		...
		if item.ItemType == "ve" && !validTVTaskNumbers(item.SeasonNumber, item.EpisodeNumber) {
			writeError(w, http.StatusBadRequest, "season_number must be non-negative and episode_number must be positive for TV tasks")
			return
		}
...
func (s *Server) handleCreateTask(w http.ResponseWriter, r *http.Request) {
	...
	if input.ItemType == "ve" && !validTVTaskNumbers(input.SeasonNumber, input.EpisodeNumber) {
		writeError(w, http.StatusBadRequest, "season_number must be non-negative and episode_number must be positive for TV tasks")
		return
	}
